...
export default (state=initialState,action)=>{
  switch(action.type){
    ...
    case LOGIN_USER_SUCCESS:
      return{...state,loading:true,user:action.eamil,uid:action.uid,isLogged:true,error:''}
    case LOGIN_USER_FAIL:
      return{...state,loading:true,email:'',error:action.payload}
    case LOGOUT_USER_START:
      return{...state,loading:true, error:''}
    case LOGOUT_USER_SUCCESS:
      console.log("ENTRATO IN LOGOUT_USER_SUCCESS");
      console.log(action.payload);
      return{...state,loadgin:false,user:null,error:''}
